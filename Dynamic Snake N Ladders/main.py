        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if restart_rect.collidepoint(mouse_pos):
                    ...
                    #Display ladder/snakes message
                    if ladder:
                        display_message("AI Climbed a Ladder!", 650, 700, BLACK)
                    elif snake:
                        display_message("AI Got Bitten by a Snake!!", 650, 700, BLACK)
                    ...
                    turn_count += 1
                    if turn_count >= 2: 
                        display_message("Board is Shuffling!!", 650, 700, BLACK)
                        snakes, ladders = generate_snakes_ladders()
                        turn_count = 0
        # Check for Win
        if human_score >= 100:
            display_message("You Win!!", 650, 500, YELLOW)
            pygame.display.update()
            pygame.time.wait(3000)
            break

        if ai_score >= 100:
            display_message("AI Wins!", 650, 700, YELLOW)
            pygame.display.update()
            pygame.time.wait(3000)
            break
